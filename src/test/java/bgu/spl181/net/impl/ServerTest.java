...
public class ServerTest {
	...
	public void Test() throws Exception {
		...
		for (int i = 0; i < numOfIterations; i++) {
			...
			ServerTest.waitForThreads();
			ServerTest.returnAllCopies();
	...
	private static void normalClient(String client) throws IOException {
		try (Socket sock = new Socket("localhost", 7777);
			...
			for (String cmd : cmds) {
				sleep((int) (Math.random() * ServerTest.randomMultConstant));
				out.write(cmd);
				out.flush();
	...
	private static void returnAllCopies() throws Exception {
		...
		for (int j = 0; j < numOfClients; j++) {
			ServerTest.clientReturn("Client" + j);
		}
	...
	private static void clientReturn(String client) throws IOException {
		try (Socket sock = new Socket("localhost", 7777);
			...
			for (String cmd : cmds) {
				sleep((long) (Math.random() * ServerTest.randomMultConstant));
				out.write(cmd);
				out.flush();
				String line = in.readLine();
				System.out.println(client + ": " + line);
